...
Rs${expense.amount}</td> 
	<td class="action" data-id="${i}" >
	<div class="edit-btn" id='${"edit-"+i}' onclick='editExpence(this)'>Edit</div>
...
function editExpence(event) {
		...
		const element = expenses[expenseIndex];
		const amount =  prompt(`Enter Amount to update for expense ${element?.name}`, element?.amount);
		expenses[expenseIndex] = { ...element, amount:parseFloat(amount)};
		renderExpenses();
}
// Add event listeners 
expenseForm.addEventListener("submit", addExpense); 
expenseList.addEventListener("click", deleteExpense); 
// expenseList.addEventListener("click", editExpence);
